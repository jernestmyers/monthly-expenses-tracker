...
function App() {
  ...
  const [uploadedData, setUploadedData] = useState<null | ResponseObject[]>(null);
  ...
  return (
    <div className="grid grid-cols-[150px_auto] grid-rows-[auto_1fr] h-screen">
            ...
            {FISCAL_YEARS.map((yr) => (
              <li key={yr}>
                <div className="flex justify-center items-center">
                  <div className={`w-2.5 h-2.5 ${yr.toString() === year ? "bg-green-500 rounded-full" : ""}`}></div>
                  <Button
      ...
      {uploadedData && (
        <SortUploadedDataDialog open={uploadedData !== null} uploadedData={uploadedData} setUploadedData={setUploadedData} />
      )}
    </div>
  );
}
